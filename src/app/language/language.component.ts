...
export class LanguageComponent implements OnInit {
  ...
  saveLanguage(): void {
    ...
    // Si estamos creando una nueva lengua, asignamos las fechas automáticamente
    const currentDate = new Date().toISOString().split('T')[0]; // Fecha en formato YYYY-MM-DD
  
    const newLanguage: any = {
      name: this.name,
      proficiency: this.proficiency,
      startDate: this.isEditMode ? this.startDate : currentDate, // Usamos la fecha actual si no estamos editando
      endDate: this.isEditMode ? this.endDate : currentDate, // Usamos la fecha actual si no estamos editando
    };
  
    // Si estamos en modo de edición, añadimos el ID
